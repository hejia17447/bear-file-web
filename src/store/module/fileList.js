export default {
	state: {
		// 表格中显示的列
		selectedColumnList: localStorage.getItem('bear_selected_column_list'),
		// 文件展示模式 0 列表模式 | 1 网格模式 | 2 时间线模式
		fileModel: localStorage.getItem('bear_file_model'),
		// 网格模式 & 时间线模式下 图标大小 单位px
		gridSize: localStorage.getItem('bear_grid_size')
			? Number(localStorage.getItem('bear_grid_size'))
			: 80,
		// 批量模式下：被选中的文件列表
		selectedFiles: [],
	...
	mutations: {
		...
		changeSelectedColumnList(state, data) {
			localStorage.setItem('bear_selected_column_list', data.toString())
		...
		changeFileModel(state, data) {
			localStorage.setItem('bear_file_model', data)
		...
		changeGridSize(state, data) {
			localStorage.setItem('bear_grid_size', data)
			state.gridSize = data
		},
		/**
		 * 设置批量操作模式下被选中的文件列表
